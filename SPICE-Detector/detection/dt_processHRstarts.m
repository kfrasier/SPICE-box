function [cParams,f] = dt_processHRstarts(fid,starts,stops,...
    p,hdr,recFile)
...
for k = 1:numStarts % stepping through using the start/end points
    ...
    % Filter the data
    filteredData = dt_getFilteredData(fid,starts(k),stops(k),hdr,...
        p,recFile);
    
    % Look for click candidates
    [clicks, noise] = dt_HR(p, hdr, filteredData);
    ...
    if ~ isempty(clicks)
        ...
        validClicks = dt_pruneClipping(clicks,p,hdr,filteredData);
        ...
        [clickDets,f] = dt_parameters(noise,filteredData,p,clicks,hdr);
